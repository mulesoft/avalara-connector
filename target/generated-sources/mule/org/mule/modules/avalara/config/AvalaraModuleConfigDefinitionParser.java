...
public class AvalaraModuleConfigDefinitionParser
    extends AbstractDevkitBasedDefinitionParser
{

    private static Logger logger = LoggerFactory.getLogger(AvalaraModuleConfigDefinitionParser.class);

    public BeanDefinition parse(Element element, ParserContext parserContext) {
        parseConfigName(element);
        BeanDefinitionBuilder builder = getBeanDefinitionBuilder(parserContext);
        builder.setScope(BeanDefinition.SCOPE_SINGLETON);
        setInitMethodIfNeeded(builder, AvalaraModuleConnectionManager.class);
        setDestroyMethodIfNeeded(builder, AvalaraModuleConnectionManager.class);
        parseProperty(builder, element, "taxServiceEndpoint", "taxServiceEndpoint");
        parseProperty(builder, element, "addressServiceEndpoint", "addressServiceEndpoint");
        parseProperty(builder, element, "batchServiceEndpoint", "batchServiceEndpoint");
    ...
    private BeanDefinitionBuilder getBeanDefinitionBuilder(ParserContext parserContext) {
        try {
            return BeanDefinitionBuilder.rootBeanDefinition(AvalaraModuleConnectionManager.class.getName());
            ...
            logger.error(("Cannot launch the mule app, the configuration [config] within the connector [avalara] is not supported in mule "+ muleVersion));
            throw new BeanDefinitionParsingException(new Problem(("Cannot launch the mule app, the configuration [config] within the connector [avalara] is not supported in mule "+ muleVersion), new Location(parserContext.getReaderContext().getResource()), null, noClassDefFoundError));
        }
    }
